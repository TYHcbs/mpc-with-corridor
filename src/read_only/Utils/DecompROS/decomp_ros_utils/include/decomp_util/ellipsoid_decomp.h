...
public:
 ...
 vec_Vecf<Dim> get_path() const { return path_; } //modified origin: vec_Vecf<Dim> , once as std::vector<Eigen::Vector3d>
  ...
  */
 void dilate(const vec_Vecf<Dim> &path, double offset_x = 0) { // modified ,origin: void dilate(const vec_Vecf<Dim> &path, double offset_x = 0)std::vector<Eigen::Vector3d> &path
...
protected:
 ...
 vec_Vecf<Dim> path_; // modified, origin vec_Vecf<Dim>, once as std::vector<Eigen::Vector3d>
 vec_Vecf<Dim> obs_;
