...
bool MPC_Class::Run(void)
{
    ...
    if (solve_flag == true && init_flag == true) {
        ...
        if (init_flag) {
            ...
            // if (diagnostics.status_msg == OSQP_DUAL_INFEASIBLE_INACCURATE) {
            //     ROS_ERROR("[MPC]: Error status: Dual Infeasible Inaccurate");
            // }
            // if (diagnostics.status_msg == OSQP_PRIMAL_INFEASIBLE_INACCURATE) {
            //     ROS_ERROR("[MPC]: Error status: Primal Infeasible Inaccurate");
            // }
            // if (diagnostics.status_msg == OSQP_SOLVED_INACCURATE) {
            //     ROS_ERROR("[MPC]: Error status: Solved Inaccurate");
            // }
            // if (diagnostics.status_msg == OSQP_SIGINT) {
            //     ROS_ERROR("[MPC]: Error status: Sigint");
            // }
            // if (diagnostics.status_msg == OSQP_MAX_ITER_REACHED) {
            //     ROS_ERROR("[MPC]: Error status: Max Iter Reached");
            // }
            // if (diagnostics.status_msg == OSQP_PRIMAL_INFEASIBLE) {
            //     ROS_ERROR("[MPC]: Error status: Primal Infeasible");
            //     // std::cout << "init state: " << X_0_.transpose() << std::endl;
            // }
            // if (diagnostics.status_msg == OSQP_DUAL_INFEASIBLE) {
            //     ROS_ERROR("[MPC]: Error status: Dual Infeasible");
            // }
            // if (diagnostics.status_msg == OSQP_NON_CVX) {
...
void MPC_Class::SetFSC(Eigen::Matrix<double, Eigen::Dynamic, 4>& planes, int step)
{
...
void MPC_Class::ProblemFormation(void)
{
...
void MPC_Class::SystemModel(Eigen::MatrixXd& A, Eigen::MatrixXd& B, double t)
{
    A.resize(9, 9);
    A.setZero();
...
void MPC_Class::ALLConstraint(mpc_osqp_t& mpc)
{
...
void MPC_Class::UpdateBound(mpc_osqp_t& mpc, const Eigen::VectorXd& x_0)
{
